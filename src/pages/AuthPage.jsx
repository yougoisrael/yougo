...
export default function AuthPage({ onDone, onGuest, onBusiness }) {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (session?.user) {
        ...
        if (meta.firstName) {
          onDone({ id: u.id, email: u.email, name: meta.firstName + " " + meta.lastName,
  ...
  async function handleEmailSubmit() {
    ...
    if (error) {
      if (error.message?.includes("rate") || error.status === 429)
        ...
        setEmailError("שגיאה: " + (error.message || "נסה שוב"));
      return;
    }
    setStep("otp");
  }
  ...
  function validateForm() {
    ...
    if (!form.age || isNaN(age) || age < 13 || age > 100) errs.age = "גיל לא תקין";
    return errs;
  }
  ...
  async function handleRegister() {
    ...
    setTimeout(() => {
      supabase.auth.getUser().then(({ data: { user } }) => {
        onDone({ id: user.id, email: user.email, name: form.firstName + " " + form.lastName,
          firstName: form.firstName, phone: form.phone, gender: form.gender, age: form.age });
      });
    }, 1500);
  }
  ...
  if (step === "splash") return (
    <div style={{ fontFamily:"Arial,sans-serif", background:"linear-gradient(160deg,#C8102E 0%,#7B0D1E 60%,#3D0511 100%)", minHeight:"100vh", maxWidth:430, margin:"0 auto", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", position:"relative", overflow:"hidden", direction:"rtl" }}>
      {/* زر دلג — أعلى يسار صغير وأنيق */}
      <button onClick={onGuest} style={{ position:"absolute", top:20, left:16, background:"rgba(255,255,255,0.12)", border:"1px solid rgba(255,255,255,0.25)", borderRadius:20, padding:"7px 16px", color:"rgba(255,255,255,0.8)", fontSize:12, fontWeight:700, cursor:"pointer", display:"flex", alignItems:"center", gap:5, animation:"splashFade 1s .8s both" }}>
        דלג ←
      </button>
  ...
  if (step === "email") return (
    <div style={{ fontFamily:"Arial,sans-serif", background:C.bg, minHeight:"100vh", maxWidth:430, margin:"0 auto", direction:"rtl", display:"flex", flexDirection:"column" }}>
      <div style={{ background:"linear-gradient(160deg,#C8102E,#9B0B22)", padding:"44px 24px 64px", position:"relative", overflow:"hidden" }}>
        <div style={{ position:"absolute", bottom:-30, left:0, right:0, height:60, background:C.bg, borderRadius:"50% 50% 0 0" }} />
        {/* دلג — أعلى يسار صغير */}
        <button onClick={onGuest} style={{ position:"absolute", top:16, left:16, background:"rgba(255,255,255,0.13)", border:"1px solid rgba(255,255,255,0.25)", borderRadius:18, padding:"6px 14px", color:"rgba(255,255,255,0.85)", fontSize:11, fontWeight:700, cursor:"pointer" }}>
          דלג ←
        </button>
        <div style={{ marginBottom:14, marginTop:4 }}><YougoLogo size={44} /></div>
        <div style={{ color:"white", fontSize:26, fontWeight:900 }}>ברוך הבא!</div>
        <div style={{ color:"rgba(255,255,255,0.8)", fontSize:13, marginTop:5 }}>הזן את האימייל שלך — נשלח קוד אימות</div>
      </div>

      <div style={{ flex:1, padding:"22px 20px 30px", overflowY:"auto" }}>

        {/* ✅ Google + Apple — فوق أولاً */}
        <div style={{ display:"flex", gap:10, marginBottom:18 }}>
          <button disabled style={{ flex:1, background:"white", border:"1.5px solid #E5E7EB", borderRadius:14, padding:"13px 8px", fontSize:13, fontWeight:600, cursor:"not-allowed", display:"flex", alignItems:"center", justifyContent:"center", gap:7, color:"#374151", opacity:0.5 }}>
            {GOOG_SVG} Google
            <span style={{ fontSize:10, color:"#9CA3AF", fontWeight:500 }}>בקרוב</span>
          </button>
          <button disabled style={{ flex:1, background:"#111", color:"white", border:"none", borderRadius:14, padding:"13px 8px", fontSize:13, fontWeight:600, cursor:"not-allowed", display:"flex", alignItems:"center", justifyContent:"center", gap:7, opacity:0.45 }}>
            {APPLE_SVG} Apple
            <span style={{ fontSize:10, color:"rgba(255,255,255,0.5)", fontWeight:500 }}>בקרוב</span>
          </button>
        </div>

        <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:16 }}>
          <div style={{ flex:1, height:1, background:"#E5E7EB" }} />
          <span style={{ color:"#9CA3AF", fontSize:12 }}>או המשך עם אימייל</span>
          <div style={{ flex:1, height:1, background:"#E5E7EB" }} />
        </div>

        {/* حقل الإيميل */}
        <div style={{ fontSize:12, color:"#6B7280", fontWeight:600, marginBottom:6 }}>כתובת אימייל</div>
        <input value={email} onChange={e => { setEmail(e.target.value); setEmailError(""); }}
          onKeyDown={e => { if (e.key === "Enter") handleEmailSubmit(); }}
          placeholder="example@email.com" type="email" autoComplete="email"
          style={{ width:"100%", background:"white", border:"1.5px solid "+(emailError?C.red:"#E5E7EB"), borderRadius:14, padding:"13px 16px", fontSize:15, outline:"none", direction:"ltr", textAlign:"left", fontFamily:"Arial,sans-serif", color:"#111827", marginBottom:4 }} />
        {emailError && <div style={{ color:C.red, fontSize:12, marginBottom:6, marginTop:2 }}>{emailError}</div>}
        <div style={{ color:"#9CA3AF", fontSize:11, marginBottom:16, marginTop:6 }}>
          משתמש קיים — תיכנס ישירות 🙂 &nbsp;|&nbsp; חדש — ניצור חשבון
        </div>

        <button onClick={handleEmailSubmit} disabled={loading}
          style={{ width:"100%", background:loading?"rgba(200,16,46,0.5)":C.red, color:"white", border:"none", borderRadius:16, padding:"15px", fontSize:15, fontWeight:900, cursor:loading?"not-allowed":"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:10, boxShadow:"0 6px 20px rgba(200,16,46,0.3)", marginBottom:16 }}>
          {loading ? <><Spinner />שולח קוד...</> : <><IcoCheck s={18} c="white" />המשך</>}
        </button>
        ...
        <div style={{ borderTop:"1px solid #F3F4F6", paddingTop:16 }}>
          ...
          <button onClick={onBusiness}
            style={{ width:"100%", background:"linear-gradient(135deg,#111827,#1f2937)", color:"white", border:"none", borderRadius:14, padding:"13px", fontSize:13, fontWeight:800, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
  ...
  if (step === "otp") return (
    <div style={{ fontFamily:"Arial,sans-serif", background:C.bg, minHeight:"100vh", maxWidth:430, margin:"0 auto", direction:"rtl", display:"flex", flexDirection:"column" }}>
      <div style={{ background:"linear-gradient(160deg,#C8102E,#9B0B22)", padding:"40px 24px 60px", position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ color:"rgba(255,255,255,0.8)", fontSize:13, marginTop:6 }}>שלחנו קוד אימות ל-<b>{email}</b></div>
      </div>
      <div style={{ flex:1, padding:"30px 24px" }}>
        <div style={{ fontSize:14, color:"#111827", fontWeight:700, marginBottom:20, textAlign:"center" }}>הזן את הקוד בן 6 הספרות</div>
        <div style={{ display:"flex", gap:8, justifyContent:"center", marginBottom:8, direction:"ltr" }}>
          {otp.map((digit, idx) => (
            <input key={idx} id={"otp-"+idx} value={digit}
              ...
              maxLength={1}
              style={{ width:46, height:56, textAlign:"center", fontSize:22, fontWeight:900, border:"2px solid "+(otpError?C.red:digit?C.red:"#E5E7EB"), borderRadius:14, outline:"none", background:digit?"rgba(200,16,46,0.05)":"white", color:otpError?"#EF4444":"#111827", fontFamily:"Arial,sans-serif" }} />
          ))}
        </div>
        {otpError && <div style={{ textAlign:"center", color:"#EF4444", fontSize:13, fontWeight:600, marginBottom:12 }}>{otpError}</div>}
        {loading && <div style={{ textAlign:"center", marginBottom:16 }}><Spinner size={24} color={C.red} /></div>}
      ...
      <div style={{ fontFamily:"Arial,sans-serif", background:C.bg, minHeight:"100vh", maxWidth:430, margin:"0 auto", direction:"rtl", display:"flex", flexDirection:"column" }}>
        ...
        <div style={{ flex:1, padding:"24px 20px", overflowY:"auto" }}>
          <div style={{ display:"flex", gap:10, marginBottom:14 }}>
            {[{ l:"שם פרטי", k:"firstName", ph:"כגון: חמדאן" }, { l:"שם משפחה", k:"lastName", ph:"כגון: אחמד" }].map(f => (
              <div key={f.k} style={{ flex:1 }}>
                <div style={{ fontSize:12, color:"#6B7280", fontWeight:600, marginBottom:5 }}>{f.l} *</div>
                <input value={form[f.k]} onChange={e => { const v=e.target.value; setForm(p=>({...p,[f.k]:v})); setFormErrors(p=>({...p,[f.k]:""})); }}
                  placeholder={f.ph}
                  style={{ width:"100%", background:"white", border:"1.5px solid "+(formErrors[f.k]?C.red:"#E5E7EB"), borderRadius:13, padding:"12px 13px", fontSize:13, outline:"none", fontFamily:"Arial,sans-serif" }} />
          ...
          <div style={{ marginBottom:14 }}>
            <div style={{ fontSize:12, color:"#6B7280", fontWeight:600, marginBottom:5 }}>מספר טלפון *</div>
            <div style={{ display:"flex", gap:8 }}>
              <div style={{ background:"white", border:"1.5px solid #E5E7EB", borderRadius:13, padding:"12px 13px", display:"flex", alignItems:"center", gap:6, flexShrink:0 }}>
              ...
              <input value={form.phone} onChange={e => { const v=e.target.value.replace(/[^\d-]/g,""); setForm(p=>({...p,phone:v})); setFormErrors(p=>({...p,phone:""})); }}
                ...
                style={{ flex:1, background:"white", border:"1.5px solid "+(formErrors.phone?C.red:"#E5E7EB"), borderRadius:13, padding:"12px 13px", fontSize:14, outline:"none", direction:"ltr", textAlign:"left", fontFamily:"Arial,sans-serif" }} />
            </div>
            {formErrors.phone && <div style={{ color:C.red, fontSize:10, marginTop:3 }}>{formErrors.phone}</div>}
          </div>

          <div style={{ marginBottom:14 }}>
            <div style={{ fontSize:12, color:"#6B7280", fontWeight:600, marginBottom:8 }}>מגדר *</div>
            <div style={{ display:"flex", gap:10 }}>
              {[{ v:"male", l:"זכר" }, { v:"female", l:"נקבה" }, { v:"other", l:"אחר" }].map(g => (
                <button key={g.v} onClick={() => { setForm(p=>({...p,gender:g.v})); setFormErrors(p=>({...p,gender:""})); }}
                  style={{ flex:1, padding:"12px 8px", borderRadius:14, border:"2px solid "+(form.gender===g.v?C.red:"#E5E7EB"), background:form.gender===g.v?"rgba(200,16,46,0.06)":"white", cursor:"pointer", fontSize:12, fontWeight:form.gender===g.v?700:500, color:form.gender===g.v?C.red:"#6B7280" }}>
                  {g.l}
                </button>
              ))}
            </div>
          ...
          <div style={{ marginBottom:24 }}>
            <div style={{ fontSize:12, color:"#6B7280", fontWeight:600, marginBottom:5 }}>גיל *</div>
            <input value={form.age} onChange={e => { const v=e.target.value.replace(/\D/g,""); setForm(p=>({...p,age:v})); setFormErrors(p=>({...p,age:""})); }}
              placeholder="(13-100)" maxLength={3}
              style={{ width:"100%", background:"white", border:"1.5px solid "+(formErrors.age?C.red:"#E5E7EB"), borderRadius:13, padding:"12px 13px", fontSize:14, outline:"none", fontFamily:"Arial,sans-serif" }} />
            {formErrors.age && <div style={{ color:C.red, fontSize:10, marginTop:3 }}>{formErrors.age}</div>}
          </div>

          <button onClick={handleRegister} disabled={loading}
            style={{ width:"100%", background:loading?"rgba(200,16,46,0.5)":C.red, color:"white", border:"none", borderRadius:16, padding:"15px", fontSize:15, fontWeight:900, cursor:loading?"not-allowed":"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:10, boxShadow:"0 6px 20px rgba(200,16,46,0.3)" }}>
            {loading ? <><Spinner />יוצר חשבון...</> : <><IcoCheck s={18} c="white" />יצירת חשבון</>}
          </button>
        </div>
        <style>{`*{box-sizing:border-box}@keyframes spin{to{transform:rotate(360deg)}}`}</style>
      </div>
